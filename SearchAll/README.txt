SearchAll Streaming Expressions Plugin

This custom developed plugin provides a streaming expression searchAll()
which returns all rows from the collection
...
3. To test the project run following command:
    ...
    - com.biologis.solr.client.solrj.io.stream.SearchAll.txt (a very brief report)
    - TEST-com.biologis.solr.client.solrj.io.stream.SearchAll.xml
...
4. To create jar file which could be used for as a plugin for SolrCloud, run following command:
    ...
    - in target folder you'll see the SearchAll-1.0-SNAPSHOT.jar file
...
3. Try the 'searchAll' expression and feel free to wrap it with the other streaming expressions
