...
public class WalkNextNode extends TupleStream implements Expressible {
    ...
    // my added parameters
    private String conj;    // for conjunction (AND/OR)
    private String direct;  // for direction (forward/backward)
    ...
    public WalkNextNode(String zkHost,
                             String collection,
    ...
    public WalkNextNode(StreamExpression expression, StreamFactory factory) throws IOException {

        String collectionName = factory.getValueOperand(expression, 0);
        List<StreamExpressionNamedParameter> namedParams = factory.getNamedOperands(expression);
        StreamExpressionNamedParameter zkHostExpression = factory.getNamedOperand(expression, "zkHost");

        List<StreamExpression> streamExpressions = factory.getExpressionOperandsRepresentingTypes(expression, Expressible.class, Tuple.class);
        // Collection name
        ...
        // -- my added parameters ------------------------------------------------------------------------------------//
        String conj = null;         // must be AND/OR variable

        StreamExpressionNamedParameter conjExpression = factory.getNamedOperand(expression, "conj");
        if(conjExpression == null) {
            throw new IOException(String.format(Locale.ROOT, "invalid expression %s - conj parameter must contain AND/OR value", expression));
        }

        String direct = null;       // must be forward/backward

        StreamExpressionNamedParameter directExpression = factory.getNamedOperand(expression, "direct");
        if(directExpression == null) {
            throw new IOException(String.format(Locale.ROOT, "invalid expression %s - direct param must contain forward/backward value", expression));
        }
        //TODO the definition will be continued
        //------------------------------------------------------------------------------------------------------------//
        ...
        for(StreamExpressionNamedParameter namedParam : namedParams) {
            if(!namedParam.getName().equals("zkHost") &&
                    ...
                    !namedParam.getName().equals("conj") &&                     // my added
                    !namedParam.getName().equals("direct"))                     // parameters
            {
    ...
    private void init(String zkHost,
        ...
        this.conj = conj;                   // added
        this.direct = direct;               // parameters
    }
    ...
    private StreamExpression toExpression(StreamFactory factory, boolean includeStreams) throws IOException {
        ...
        if(includeStreams && !(tupleStream instanceof NodeStream)) {
            if(tupleStream instanceof Expressible) {
                ...
                throw new IOException("This WalkThrough contains non-expressible TupleStream - it converted to an expression");
            }
        } else {
        ...
        // -- my added parameters ------------------------------------------------------------------------------------//
        expression.addParameter(new StreamExpressionNamedParameter("conj", nodeStream.toString(conj)));
        expression.addParameter(new StreamExpressionNamedParameter("direct", nodeStream.toString(direct)));
        //TODO shall be tested: if it's nodeStream variable
        //#-----------------------------------------------------------------------------------------------------------//
    ...
    public Explanation toExplanation(StreamFactory factory) throws IOException {
        ...
        explanation.setExpressionType(Explanation.ExpressionType.GRAPH_SOURCE);
        explanation.setExpression(toExpression(factory).toString());

        // one child is a stream
        explanation.addChild(tupleStream.toExplanation(factory));

        // one child is a dataset so add it at this point
    ...
    public void setStreamContext(StreamContext context) {
        ...
        if(traversal == null) {
            ...
            // This ensures that two separate traversals in the same expression do not pollute each others traversal
    ...
    @Override
    //TODO: find out entry points for parameters conj() & direct()
    // **----------------------------- seems the logic starts there -----------------------------** //
    ...
    private class JoinRunner implements Callable<List<Tuple>> {
        ...
        public List<Tuple> call() {
            ...
            TupleStream stream = null;
            try {
                stream = new UniqueStream(new CloudSolrStream(zkHost, collection, joinSParams), new MultipleFieldEqualitor(new FieldEqualitor(gather)));
    ...
    //TODO: find out entry points for parameters conj() & direct()
    // **---------------------- another potential entry point for the logic --------------------** //
